...
namespace emulator
{
    ...
    union InterruptRegister
    {
        ...
        struct
        {
            uint8_t interrupt_enable_v_blank:1; // 0xFF0F bit 0
            uint8_t interrupt_enable_lcd_stat:1;// 0xFF0F bit 1
            uint8_t interrupt_enable_timer:1;   // 0xFF0F bit 2
            uint8_t interrupt_enable_serial:1;  // 0xFF0F bit 3
            uint8_t interrupt_enable_joypad:1;  // 0xFF0F bit 4
            uint8_t unused47:3;                 // 0xFF0F bit 5..7
        };
    };
    ...
    union MemoryMap
    {
        ...
        struct
        {
            uint8_t rom_bank[0x4000];           // 0000-3FFF   16KB ROM Bank 00     (in cartridge, fixed at bank 00)
            uint8_t switch_rom_bank[0x4000];    // 4000-7FFF   16KB ROM Bank 01..NN (in cartridge, switchable bank number)
            uint8_t video_ram[0x2000];          // 8000-9FFF   8KB Video RAM (VRAM) (switchable bank 0-1 in CGB Mode)
            uint8_t external_ram[0x2000];       // A000-BFFF   8KB External RAM     (in cartridge, switchable bank, if any)
            uint8_t ram_bank_0[0x1000];         // C000-CFFF   4KB Work RAM Bank 0 (WRAM)
            uint8_t ram_bank_1[0x1000];         // D000-DFFF   4KB Work RAM Bank 1 (WRAM)  (switchable bank 1-7 in CGB Mode)
            uint8_t echo_ram_bank[0x1E00];      // E000-FDFF   Same as C000-DDFF (ECHO)    (typically not used)
            uint8_t sprite_attr_table[0xA0];    // FE00-FE9F   Sprite Attribute Table (OAM)
            uint8_t io_map_area_1[0x60];        // FEA0-FEFF   Not Usable
            IoPorts io_ports;                   // FF00-FF7F   I/O Ports
            uint8_t io_internal_ram[0x7F];      // FF80-FFFE   High RAM (HRAM)
            uint8_t interrupt_enable_v_blank:1; // 0xFF0F bit 0
            uint8_t interrupt_enable_lcd_stat:1;// 0xFF0F bit 1
            uint8_t interrupt_enable_timer:1;   // 0xFF0F bit 2
            uint8_t interrupt_enable_serial:1;  // 0xFF0F bit 3
